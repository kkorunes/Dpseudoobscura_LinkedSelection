...
# 
#
# USAGE: perl klk_matchToRecombRates.pl fourfoldfile RecRateFile gene_span_starts.txt
#
# #######################################################################################
use strict;
...
my $output = "chr"."$chr"."_fourfolddegeneratepi_and_recombrates\.txt";
open (OUT, ">$output") or die "file not found $!";
print OUT "CHR\tGENE\tAVG_PI\tFOURFOLD_POSITIONS_COUNTED\tREC_RATE\tGENE_START\n";
...
while (<INTRON>){
	...
	if (exists ($recRates{$geneName})){
		...
		$start = "$saved[0]";
		$recRate = "$saved[1]";
		if (exists ($printed{$geneName})){
			print "Already encountered $geneName, now on:\t$line\n\tSaved: @saved\n";
		}else{
			$printed{$geneName}=1;
		}
	} else {
